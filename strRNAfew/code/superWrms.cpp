/*
c++ superWrms.cpp -o superWrms sims/util.o sims/geom.o -lm

code/superWrms pairs.dat model.pdb true.pdb secs.dat 
...
float	pdbout ( Prot*, Prot*, double**, double**, int, double*, double*, double**, char* ); 
...
int	nbump = 0;
	...
	FIR(i,len) { int s = read_line(sec,line);
		if ( s <= 0) break;
		sscanf(line,"%d %d", &n, &s);
		if ( s>1) s = 1;
		known->acc[i] = (float)s;
	...
	printf("RMS   Unsmoothed %6.3f  Smoothed 1x %6.3f  Smoothed 2x %6.3f\n", rms.x, rms.y, rms.z);
}
...
int	cycles = 1;
	...
	FOR(i,3) rot[i] = (double*)alloca(sizeof(double)*3);
	...
	// Unsmoothed
	FOR(i,len) ww[i] = a->acc[i] = b->acc[i];
	rmsw.x = supermac(ww,va,vb,len,ac,bc,rot);
	rms.x = pdbout(a,b,va,vb,len,ac,bc,rot,"super0.pdb");
	...
	rmsw.y = supermac(ww,va,vb,len,ac,bc,rot);
	rms.y = pdbout(a,b,va,vb,len,ac,bc,rot,"super1.pdb");
	...
	rmsw.z = supermac(ww,va,vb,len,ac,bc,rot);
	rms.z = pdbout(a,b,va,vb,len,ac,bc,rot,"super2.pdb");
	if (rms.z>99.999) rms.z = 99.999;
	return rms;
}

float pdbout ( Prot *a, Prot *b, double **va, double **vb, int len, double *ac, double *bc, double **rot, char *file ) 
{
FILE	*pdb;
float	rms, sum = 0.0;
Vec	acnt, bcnt;
Mat	mat;
	...
	for (int i=0; i<=len+1; i++) a->cas[i] -= acnt;
	for (int i=0; i<=len+1; i++) {
		b->cas[i] -= bcnt;
		b->cas[i] = mat*b->cas[i];
	}
	FIR(i,len)
	{ float d = a->cas[i] | b->cas[i]; d *= d;
		sum += d;
	}
	rms = sqrt(sum/(float)len);	
	pdb = fopen(file,"w");
	FIR(i,len) { float w = a->acc[i];
		fprintf(pdb,"ATOM%7d  P     G A%4d     %7.3f %7.3f %7.3f  0.00 %5.2f\n", i,i,
			a->cas[i].x, a->cas[i].y, a->cas[i].z, w);
	}
	fprintf(pdb,"TER\n");
	FIR(i,len) { float w = b->acc[i];
		fprintf(pdb,"ATOM%7d  P     G B%4d     %7.3f %7.3f %7.3f  0.00 %5.2f\n", i,i,
			b->cas[i].x, b->cas[i].y, b->cas[i].z, w);
	}
	fprintf(pdb,"TER\n");
	fclose(pdb);
	return rms;
 ...
 * (FOR NOT use the Row value returned by siva_setup()! )
