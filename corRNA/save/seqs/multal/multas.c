...
{	int	i, j, k;
	...
	char	*c;
	FILE	*seq, *fas;
	seq = fopen("seedaln.seq","w");
	fas = fopen("seedaln.fas","w");
	Pi(nblocks) NL
	for ( i = 0; i < nblocks; i++ ) 
	{ 	Blocks	*b = block[i];
...
{ int   *posa = al->position_a, *posb = al->position_b;
 Blocks *blka = cluster->blk[0];
 Blocks *blkb = cluster->blk[1];
printf("%4d %4d   ",posb[i],posa[i]);
for(j=0; j<blka->members; j++) { char r = ' ';
if (posa[i]) r = blka->seq[j]->residue[posa[i]];
printf("%c",r); }
SP
for(j=0; j<blkb->members; j++) { char r = ' ';
if (posb[i]) r = blkb->seq[j]->residue[posb[i]];
printf("%c",r); }
NL
}}
...
if (a<1 || a>blk_a->length) alignment[cell] = 'a'; else alignment[cell] = s->residue[a];
	...
	{	int	i;
		...
		{	Seqs	*s = blk_b->seq[i];		if(alignment[cell] != ' ' && alignment[cell] != s->residue[b])
			...
			alignment[cell] = s->residue[b];	if(cell >= aln_len || col < 0 || row < 0 ) 
									printf("*** alignment overflow *** %d %d\n",row,col);
if (b<1 || b>blk_b->length) alignment[cell] = 'b'; else alignment[cell] = s->residue[b];
			if ( col > colmax[row] ) colmax[row] = col;
			cell += leng;
			row++;
		}
