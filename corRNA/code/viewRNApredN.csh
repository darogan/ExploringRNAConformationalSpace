...
@ n = `cat $argv[2] | wc -l`
# select cut
#cat bondM.tmp | awk -v n=$n '{m=20+n-$8*45; if(m>0){print $1, $2, m}}' > bond1.tmp
cat bondM.tmp | awk -v n=$n '{m=20+n-$8*4; if(m>0){print $1, $2, m}}' > bond1.tmp
#cat bondM.tmp | awk -v n=$n '{m=n-$8*3.5; if(m>0){print $1, $2, m}}' > bond1.tmp
#cat bondM.tmp | awk -v n=$n '{m=n-$8*3.33; if(m>0){print $1, $2, m}}' > bond1.tmp
#cat bondM.tmp | awk -v n=$n '{m=n-$8*3; if(m>0){print $1, $2, m}}' > bond1.tmp
cat bond1.tmp | awk '{printf("select %d,%d\nhbond %d\ncolour hbonds blue \n", $1, $2, $3)}' > bond1.ras
# select cut
#cat bondN.tmp | awk -v n=$n '{m=20+n-$8*4.5; if(m>0){print $1, $2, m}}' > bond2.tmp
cat bondN.tmp | awk -v n=$n '{m=20+n-$8*4; if(m>0){print $1, $2, m}}' > bond2.tmp
#cat bondN.tmp | awk -v n=$n '{m=n-$8*3.5; if(m>0){print $1, $2, m}}' > bond2.tmp
#cat bondN.tmp | awk -v n=$n '{m=n-$8*3.33; if(m>0){print $1, $2, m}}' > bond2.tmp
#cat bondN.tmp | awk -v n=$n '{m=n-$8*3; if(m>0){print $1, $2, m}}' > bond2.tmp
...
if ( -e reds.ras ) then
	echo Adding red bonds
	cat reds.ras >> temp.ras
endif
